...
###################################################### FACE RECOG SECTION #########################################################
    ...
    st.write("--> Click on start recognition to turn on camera")
    ...
    known_face_names = ["Chris","Robert","Saudhey"]
    ...
    if recog:
        ...
        cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)        #Gets a refernce to webcam 0
        ...
        while cap.isOpened():
            ...
            rgb_small_frame = cv2.cvtColor(small_frame, cv2.COLOR_BGR2RGB)      #cnvts to RGB
...
##################################################FACE MESHING################################################
    ...
    st.write("--> Upload an image to construct a mesh")
    if image_file_path is not None:
        image = np.array(Image.open(image_file_path))
        st.sidebar.image(image) 
...
####################################### BACKGROUND ##################################################
    ...
    st.write("--> Upload an image to set it as your background")
    if image_file_path is not None:
